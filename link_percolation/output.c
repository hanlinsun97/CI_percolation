...
void save_averaged_results(const char *filename, int n, int m,
  ...
  online_stats_get_std(core_stats, core_std);

  fprintf(f, "# Link Removal Percolation Results (Averaged)\n");
  fprintf(f, "# N=%d M=%d\n", n, m);
...
static void ebe_writer_write_header(FILE *f, int n, double c, int M_graphs,
                                    int M_realizations, int total_runs) {
  fprintf(f, "# Event-Based Ensemble (EBE) Results for link removal\n");
  fprintf(f, "# N=%d c=%.4f\n", n, c);
  fprintf(f, "# M_graphs=%d M_realizations=%d Total_runs=%d\n", M_graphs,
  ...
  fprintf(f, "#(25) core_p* (26) core_drop (27) core_size* (28) core_zero_p\n");
}
...
EBEWriter *ebe_writer_begin(const char *filename, int n, double c,
  ...
  ebe_writer_write_header(f, n, c, M_graphs, M_realizations, total_runs);
...
void ebe_writer_record(EBEWriter *writer, int run_index,
  ...
  fprintf(writer->file, "%.8f %.8f ", d->p1, d->p2);
  fprintf(writer->file, "%.8f %d %d %.8f\n", d->core_p_star, d->core_drop,
          d->core_size_star, d->core_zero_p);
  ...
  writer->sum_core_p += d->core_p_star;
  writer->runs_written++;
}
...
void ebe_writer_finish(EBEWriter *writer) {
  ...
  if (writer->file) {
    ...
    if (writer->runs_written > 0) {
      ...
      fprintf(writer->file, "# Mean core p* = %.8f\n",
              writer->sum_core_p * inv);
    }
  }
...
void save_enhanced_ebe_results(const char *filename, int n, double c,
                               ...
                               int total_runs) {
  EBEWriter *writer =
      ebe_writer_begin(filename, n, c, M_graphs, M_realizations, total_runs);
  if (!writer)
    return;
