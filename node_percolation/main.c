...
static void print_usage(const char *prog) {
  printf("Usage: %s -g <input|ER> -s <strategy> [options]\n", prog);
  printf("Required:\n");
  printf("  -g TYPE       Graph type: input|ER\n");
  printf("  -s STRATEGY   Ranking strategy: ci|random\n");
  printf("If -g input:  -f FILE\n");
  printf("If -g ER:     -N NUM -c AVG_DEGREE\n");
  printf("Options:\n");
  printf("  -L RADIUS          CI radius (default 0)\n");
  ...
  printf("  -o PREFIX          Output prefix (default NR)\n");
...
int main(int argc, char **argv) {
  ...
  RankingConfig ranking = {.strategy = "ci", .ci_radius = 0};
  int M_graphs = 1;
  int M_realizations = 1;
  char output_prefix[128] = "NR";
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-h") == 0) {
      ...
      ranking.strategy = argv[++i];
    } else if (strcmp(argv[i], "-L") == 0 && i + 1 < argc) {
      ranking.ci_radius = atoi(argv[++i]);
  ...
  if (mode == GRAPH_ER) {
    snprintf(averaged_file, sizeof(averaged_file),
             "%s_NR_ER_N%d_c%.2f_%s_L%d_Mg%d_Mr%d.dat", output_prefix,
             graph_nodes, avg_degree, ranking.strategy, ranking.ci_radius,
             M_graphs, M_realizations);
    snprintf(ebe_file, sizeof(ebe_file),
             "%s_EBE_NR_ER_N%d_c%.2f_%s_L%d_Mg%d_Mr%d.dat", output_prefix,
             graph_nodes, avg_degree, ranking.strategy, ranking.ci_radius,
             M_graphs, M_realizations);
  } else {
    snprintf(averaged_file, sizeof(averaged_file),
             "%s_NR_INPUT_%s_%s_L%d_Mg%d_Mr%d.dat", output_prefix,
             graph_label, ranking.strategy, ranking.ci_radius, M_graphs,
             M_realizations);
    snprintf(ebe_file, sizeof(ebe_file),
             "%s_EBE_NR_INPUT_%s_%s_L%d_Mg%d_Mr%d.dat", output_prefix,
             graph_label, ranking.strategy, ranking.ci_radius, M_graphs,
             M_realizations);
  }

  EBEWriter *ebe_writer =
      ebe_writer_begin(ebe_file, graph_nodes, ranking.strategy, M_graphs,
  ...
  NodeRemovalConfig config = {.ranking = ranking};
...
#endif
  {
    double *local_gc = (double *)malloc(series_length * sizeof(double));
    double *local_small = (double *)malloc(series_length * sizeof(double));
    double *local_lambda = (double *)malloc(series_length * sizeof(double));
    double *local_comp = (double *)malloc(series_length * sizeof(double));
    double *local_cycles =
        (double *)malloc(series_length * sizeof(double));
    NodePercolationResult *thread_result =
        create_node_percolation_result(graph_nodes);

    if (!local_gc || !local_small || !local_lambda || !local_comp ||
        !local_cycles || !thread_result) {
#ifdef _OPENMP
#pragma omp critical
#endif
      { error_flag = 1; }
...
#pragma omp critical
        ...
        for (int realization = 0; realization < M_realizations;
             realization++) {
          if (error_flag)
            break;
          int run = graph_idx * M_realizations + realization;
          reset_node_percolation_result(thread_result);
          uint64_t run_seed =
              seed ^ (0x9e3779b97f4a7c15ULL * (uint64_t)(run + 1));
          PseudoCriticalPoints ecp;
          if (run_node_percolation(current_graph, &config, thread_result, &ecp,
                                   run_seed) != 0) {
#ifdef _OPENMP
#pragma omp critical
#endif
          ...
          for (int i = 0; i < sample_length; i++) {
            local_gc[i] = thread_result->giant_size[i] / (double)graph_nodes;
          ...
          record.p_plus = ecp.p_plus;
          record.p_plus_jump = ecp.p_plus_jump;
          record.q_plus = ecp.q_plus;
          record.q_zero = ecp.q_zero;
          record.q_minus = ecp.q_minus;
          record.p1 = ecp.p1;
          record.p2 = ecp.p2;
          record.P_p_plus = ecp.P_at_p_plus;
          record.s_p_plus = ecp.s_at_p_plus;
          record.nc_p_plus = ecp.nc_at_p_plus;
          record.nl_p_plus = ecp.nl_at_p_plus;
          record.P_q_plus = ecp.P_at_q_plus;
          record.s_q_plus = ecp.s_at_q_plus;
          record.nc_q_plus = ecp.nc_at_q_plus;
          record.nl_q_plus = ecp.nl_at_q_plus;
          record.P_q_zero = ecp.P_at_q_zero;
          record.s_q_zero = ecp.s_at_q_zero;
          record.nc_q_zero = ecp.nc_at_q_zero;
          record.nl_q_zero = ecp.nl_at_q_zero;
          record.P_q_minus = ecp.P_at_q_minus;
          record.s_q_minus = ecp.s_at_q_minus;
          record.nc_q_minus = ecp.nc_at_q_minus;
          record.nl_q_minus = ecp.nl_at_q_minus;
...
#pragma omp critical
    ...
    free(local_comp);
    free(local_cycles);
    free_node_percolation_result(thread_result);
  }
  ...
  save_averaged_results(averaged_file, graph_nodes, ranking.strategy,
                        M_graphs, M_realizations, gc_stats, small_stats,
                        lambda_stats, comp_stats, cycles_stats);
